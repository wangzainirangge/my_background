          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <span v-if="scope.row.identityLevel==1">运营者</span>
                <span v-if="scope.row.identityLevel==2">管理员</span>
              </template>
            </el-table-column>
            <el-table-column
              align="center"
...
<script>
  export default {
    ...
    methods: {
      getManageList(){
        this.$get(this.$api.module.operationManage + '?identityLevel=-1&status=-1&pageSize=' + this.pageSize + "&pageNo=" + this.pageNo).then(res => {
          if (res.status == 200){
      ...
      clickEnabled(v){
        ...
        this.$patch(this.$api.module.operationManage + '/'+v.manageID,this.updateData).then(res => {
          if (res.status == 200){
            this.getManageList();
            this.$message({
              message: '禁用成功！',
      ...
      clickSearch(){
        this.$get(this.$api.module.operationManage + '/property?username=' + this.requestData.username
      ...
      clickUpdate(v){
        this.$router.push({name: 'addBackgroundUser',params:{updateData: v}})
      },
      clickDelete(v){
        this.$drop(this.$api.module.operationManage + '/'+v.manageID).then(res => {
          if (res.status == 200){
            this.getManageList();
            this.$message({
              message: '删除成功！',
      ...
      handleSizeChange(val){
        this.pageSize = val;
        if (this.requestData.username!=''||this.requestData.realName!='') {
          this.clickSearch();
        }else {
          this.getManageList();
        }
