  ...
  export default {
    ...
    methods: {
      getInternationalList(){
        this.$get(this.$api.module.operationNews + '?status=2&type=5&pageSize=' + this.pageSize + "&pageNo=" + this.pageNo).then(res => {
          if (res.status == 200){
      ...
      updateNews(v){
        //alert(v)
        this.$router.push({name: 'addFinancialNews',params:{updateData: this.formData[v]}})
      },
      deleteNews(v){
      ...
      clickSearch(){
        ...
        this.$get(this.$api.module.operationNews + '/property?title=' + this.searchData.title
          ...
          + '&status=2&type=5&pageSize=' + this.pageSize
