  ...
  <div id="login">
    <div class="loginConbox">
      ...
      <div class="loginBox">
        <div class="loginCon">
          <p class="title"></p>
          <p class="title">新媒体后台管理系统</p>
          <el-card shadow="always" class="login-module" v-if="smdl" >
            <div slot="header" class="clearfix formTitlt" style="text-align: center">
              <span>密码登录</span>
            ...
            <el-form :model="loginForm" status-icon label-width="100px" class="demo-ruleForm">
              ...
              <el-form-item>
                <el-radio v-model="loginForm.identityLevel" label="1">运营者</el-radio>
                <el-radio v-model="loginForm.identityLevel" label="2">管理员</el-radio>
              </el-form-item>
              <el-form-item>
  ...
  methods: {
    submitForm () {
      ...
      if (this.loginForm.username === '' || this.loginForm.password === '') {
        ...
        this.$post(this.$api.module.operationManage + "/login",this.loginForm).then(res => {
          if (res.status == 200){
            this.resultData = res.data;
            that.setCookie('manageID',this.resultData.manageID,1);
            this.$message({
              message: '登陆成功！',
              type: 'success'
            });
            ...
            that.$store.dispatch('setToken', that.loginForm.username).then(() => {
              that.$router.push({path: '/Layout',params:{loginForm: that.loginForm}})
            }).catch(res => {
              that.$message({
                showClose: true,
                message: res,
        ...
        /*this.$post(this.$api.module.Test,this.loginForm).then(response => {
          if (response=="success"){
            alert(response)
            // 将 username 设置为 token 存储在 store，仅为测试效果，实际存储 token 以后台返回为准
            that.$store.dispatch('setToken', that.loginForm.username).then(() => {
